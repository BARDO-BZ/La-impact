...
(() => {
  ...
  function readEmbeddedState() {
    try {
      var el = document.getElementById('__image_slots_state');
      if (el && el.textContent && el.textContent.trim()) return JSON.parse(el.textContent);
    } catch (e) {}
    return null;
  }
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .catch(() => null)
      .then((j) => {
        // Standalone/offline: si el fetch no trajo nada, usar el estado embebido.
        if (!j || typeof j !== 'object') j = readEmbeddedState();
